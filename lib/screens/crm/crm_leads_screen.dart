...
class _CrmLeadsScreenState extends State<CrmLeadsScreen> {
  ...
  void _showLeadCallSummary(String leadId, String leadName) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _LeadCallSummarySheet(leadId: leadId, leadName: leadName),
    );
  }
          ...
          Row(
            children: [
              ...
              _buildActionButton(
                ...
                AppColors.success,
                () => _showLeadCallSummary(leadId, customerName),
              ),
            ],
          ),
        ],
...
// ── Lead Call Summary Sheet ────────────────────────────────────────────────

class _LeadCallSummarySheet extends StatefulWidget {
  final String leadId;
  final String leadName;
  const _LeadCallSummarySheet({required this.leadId, required this.leadName});
  @override
  State<_LeadCallSummarySheet> createState() => _LeadCallSummarySheetState();
}

class _LeadCallSummarySheetState extends State<_LeadCallSummarySheet> {
  final _api = ApiService();
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadCalls() async {
    try {
      final res = await _api.getCalls(leadId: widget.leadId);
      final raw = res.data;
              ...
              : (_error != null || _calls.isEmpty)
                  ? Center(
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(Icons.mic_off_rounded, size: 40, color: AppColors.textHint),
                        const SizedBox(height: 12),
                        Text(
                          _error ?? 'No calls uploaded for this lead yet',
                  ...
                  : ListView(
                      ...
                      children: [
                          ...
                          SingleChildScrollView(
                            ...
                            child: Row(
                              children: _calls.asMap().entries.map((e) {
                                final cId = e.value['id'].toString();
                                final selected = cId == _selectedCallId;
                                return GestureDetector(
                        ...
                        if (_detail != null) ...[
                          ...
                          _buildSectionScores(_detail!),
                          _buildSummary(_detail!),
                          _buildKeyPoints(_detail!),
                          if (_noContent(_detail!))
                            Container(
        ...
        child: Center(
          child: score != null
              ? Column(mainAxisSize: MainAxisSize.min, children: [
                  Text('${score.round()}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: scoreColor)),
                  Text('%', style: TextStyle(fontSize: 10, color: scoreColor.withValues(alpha: 0.7))),
                ])
              : Icon(status == 'PROCESSING' ? Icons.hourglass_top : Icons.schedule, size: 22, color: AppColors.textHint),
        ),
      ),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  value: secScore / 100, minHeight: 6,
