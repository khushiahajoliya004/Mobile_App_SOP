...
class _AiCallConfigScreenState extends State<AiCallConfigScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      List<dynamic> nums = numRaw is List
          ? numRaw
          : (numRaw is Map ? (numRaw['data'] ?? []) : []);

      final catRaw = results[1].data;
      List<dynamic> cats = catRaw is List
          ? catRaw
          : (catRaw is Map ? (catRaw['data'] ?? []) : []);
    ...
    return _items
        .where(
          (n) =>
              ...
              (n['category']?['name'] ?? '').toString().toLowerCase().contains(
                q,
              ),
        )
        .toList();
  ...
  Future<void> _toggleStatus(Map<String, dynamic> item) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(
          '${newStatus == 'ACTIVE' ? 'Activate' : 'Deactivate'} "${item['name']}"?',
        ),
        content: Text(
          'Are you sure you want to ${newStatus == 'ACTIVE' ? 'activate' : 'deactivate'} this AI call number?',
        ),
        actions: [
          TextButton(
    ...
    try {
      await _api.updateAiCallNumber(item['id'] as String, {
        'status': newStatus,
      });
      _msg('Status updated');
      _load();
        ...
        Container(
          ...
          child: Row(
            children: [
              const Expanded(
                child: Text(
                  'Manage AI calling numbers & scripts',
                  style: TextStyle(
                    fontSize: 12,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Tap "Add" to configure your first AI call number',
                  ...
                  child: Column(
                    ...
                    children: [
                      _field(
                        'Name',
                        _nameCtrl,
                        hint: 'e.g. Hyundai Call Feedback',
                        required: true,
                      ),
                      const SizedBox(height: 14),
                      _field(
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          hintText:
                              'e.g. You are Priya, an assistant for Hyundai. Speak warmly and professionally.',
