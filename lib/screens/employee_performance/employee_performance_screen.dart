...
class _EmployeePerformanceScreenState extends State<EmployeePerformanceScreen> {
  ...
  String? get _toDate => _period == 'all'
      ? null
  ...
  Future<void> _loadDetail(String userId, String name) async {
    ...
    try {
      final res = await _api.getEmployeePerformanceDetail(
        userId,
        fromDate: _fromDate,
        toDate: _toDate,
      );
      _detail = res.data is Map
  ...
  Future<void> _loadAiTrend({bool refresh = false}) async {
    ...
    try {
      final res = await _api.getEmployeeAiTrend(
        _detailUserId!,
        refresh: refresh,
      );
      _aiTrend = res.data is Map
          ? Map<String, dynamic>.from(res.data['data'] ?? res.data)
          : null;
    } catch (_) {
      _aiTrend = null;
          ...
          child: _loading
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemCount:
                        _employees.length + (_page < _totalPages ? 1 : 0),
                    itemBuilder: (_, i) {
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                ...
                ..._statusOptions.map(
                  (s) => _statusChip(s, s[0] + s.substring(1).toLowerCase()),
                ),
              ],
            ),
          ),
  ...
  Widget _detailView() {
    final d = _detail!;
    final strengths =
        (d['topStrengths'] as List?)?.map((e) => e.toString()).toList() ?? [];
    final improvementAreas =
        (d['improvementAreas'] as List?)?.map((e) => e.toString()).toList() ??
        [];
            ...
            children: [
              ...
              TextButton.icon(
                onPressed: _loadingAi
                    ? null
                    : () => _loadAiTrend(refresh: _aiTrend != null),
                icon: _loadingAi
                    ? const SizedBox(
                        width: 14,
                        height: 14,
                ...
                label: Text(
                  _aiTrend == null ? 'Performance Trend' : 'Refresh',
                  style: const TextStyle(
                    fontSize: 12,
                    color: AppColors.primary,
                    fontWeight: FontWeight.w600,
                      ...
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: _metricCard(
                              'Avg Score',
                              (d['averageScore'] as num?)?.toStringAsFixed(1) ??
                                  '0',
                              AppColors.success,
                            ),
                          ),
                          ...
                          Expanded(
                            child: _metricCard(
                              'Success',
                              '${(d['successRate'] as num?)?.toStringAsFixed(0) ?? '0'}%',
                              AppColors.warning,
                            ),
                          ),
                        ],
  ...
  Widget _buildAiTrendCard() {
    final t = _aiTrend!;
    final summary =
        t['summary']?.toString() ?? t['trendSummary']?.toString() ?? '';
    final trajectory =
        t['trajectory']?.toString() ??
        t['performanceTrajectory']?.toString() ??
        '';
    final recs = (t['recommendations'] as List?) ?? [];
    final concerns =
        (t['keyInsights'] as List?) ?? (t['concerns'] as List?) ?? [];
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
