...
class _CrmContactDetailScreenState extends State<CrmContactDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    await Future.wait([
      _api.getCrmActivities(contactId: widget.contactId, limit: 50).then((res) {
        final raw = res.data;
      ...
      }).catchError((_) { _activities = []; }),
      _api.getCrmDeals(search: _contact?['phone'] as String?).then((res) {
        final raw = res.data;
        _deals = raw is List
            ? raw.map((e) => Map<String, dynamic>.from(e)).toList()
            : ((raw is Map ? (raw['data'] ?? []) : []) as List).map((e) => Map<String, dynamic>.from(e)).toList();
      }).catchError((_) { _deals = []; }),
      _api.getCrmFollowUpsByContact(widget.contactId).then((res) {
        final raw = res.data;
        _followUps = raw is List
            ? raw.map((e) => Map<String, dynamic>.from(e)).toList()
            : ((raw is Map ? (raw['data'] ?? []) : []) as List).map((e) => Map<String, dynamic>.from(e)).toList();
      }).catchError((_) { _followUps = []; }),
      _api.getCrmPipelines().then((res) {
        final raw = res.data;
        _pipelines = raw is List
            ? raw.map((e) => Map<String, dynamic>.from(e)).toList()
            : ((raw is Map ? (raw['data'] ?? []) : []) as List).map((e) => Map<String, dynamic>.from(e)).toList();
      }).catchError((_) { _pipelines = []; }),
    ]);
  ...
  Future<void> _createNewDeal() async {
    if (_pipelines.isEmpty) { _msg('No pipelines configured', error: true); return; }
    final pipeline = _pipelines.firstWhere((p) => p['isDefault'] == true, orElse: () => _pipelines.first);
    ...
    try {
      ...
      if (mounted && newDealId != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => CrmDealDetailScreen(dealId: newDealId!, dealName: contactName)));
      }
        ...
        builder: (ctx, setSheet) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () async {
                  final d = await showDatePicker(context: ctx, initialDate: scheduledAt, firstDate: DateTime.now(), lastDate: DateTime(2030));
  ...
  Color _activityColor(String? type) {
    switch (type) {
      ...
      case 'LEAD_CREATED': return const Color(0xFF8B5CF6);
      case 'STAGE_CHANGE': return AppColors.warning;
  ...
  String _formatTypeName(String? type) {
    ...
    return type.replaceAll('_', ' ').split(' ').map((w) => w.isEmpty ? '' : '${w[0].toUpperCase()}${w.substring(1).toLowerCase()}').join(' ');
  }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            CircleAvatar(
              ...
              child: Text(name.isNotEmpty ? name[0].toUpperCase() : '?', style: TextStyle(color: stageColor, fontWeight: FontWeight.w800, fontSize: 18)),
            ),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (company.isNotEmpty) ...[
                ...
                Text('$company', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
              ],
              if (profession.isNotEmpty) ...[
                const SizedBox(height: 1),
                Text('$profession', style: const TextStyle(fontSize: 11, color: AppColors.textHint)),
              ],
              ...
              Row(children: [
                ...
                if (ownerName.isNotEmpty) ...[
                  ...
                  Flexible(child: Text(ownerName, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary), overflow: TextOverflow.ellipsis)),
                ],
              ]),
          ...
          Wrap(spacing: 6, runSpacing: 4, children: [
            if (phone.isNotEmpty) _infoChip(Icons.phone_rounded, phone, AppColors.success),
            if (email.isNotEmpty) _infoChip(Icons.email_rounded, email, AppColors.accent),
            if (altPhone.isNotEmpty) _infoChip(Icons.phone_outlined, altPhone, AppColors.textSecondary),
            if (source.isNotEmpty) Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5),
              decoration: BoxDecoration(color: AppColors.surfaceLight, borderRadius: BorderRadius.circular(8)),
              child: Text('$source', style: const TextStyle(fontSize: 10, color: AppColors.textSecondary, fontWeight: FontWeight.w600)),
            ),
          ]),
          // Address line
          if (city.isNotEmpty || address.isNotEmpty) ...[
            ...
            Row(children: [
              ...
              Flexible(child: Text([address, city].where((s) => s.isNotEmpty).join(', '), style: const TextStyle(fontSize: 11, color: AppColors.textHint), overflow: TextOverflow.ellipsis)),
            ]),
          ],
          const SizedBox(height: 10),
          // Tabs
          SingleChildScrollView(
            ...
            child: Row(children: ['Timeline', 'Deals', 'Follow-ups'].asMap().entries.map((e) {
  ...
  Widget _headerBtn(IconData icon, Color bg, Color fg, VoidCallback onTap, {String? tooltip}) => GestureDetector(
    onTap: onTap,
    child: Container(width: 34, height: 34, decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(9)), child: Icon(icon, size: 16, color: fg)),
  );
  ...
  Widget _infoChip(IconData icon, String text, Color color) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 4),
      Text(text, style: TextStyle(fontSize: 11, color: color, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis),
    ]),
  );
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(
                  ...
                  child: Text(_formatTypeName(type), style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: color)),
                ),
                if (userName.isNotEmpty) ...[
                  const SizedBox(width: 6),
                  Text(userName, style: const TextStyle(fontSize: 10, color: AppColors.textSecondary, fontWeight: FontWeight.w600)),
                ],
                const Spacer(),
              ...
              if (notes.toString().isNotEmpty) ...[
                const SizedBox(height: 4),
                Text('$notes', style: const TextStyle(fontSize: 12, color: AppColors.textPrimary), maxLines: 3, overflow: TextOverflow.ellipsis),
              ],
              if (outcome.toString().isNotEmpty) ...[
                const SizedBox(height: 2),
                Text('→ $outcome', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary, fontStyle: FontStyle.italic)),
              ],
            ])),
          ]),
        );
    ...
    return Column(children: [
      if (_deals.isNotEmpty) ...[
        Padding(
          ...
          child: Row(children: [
            Text('${_deals.length} deal${_deals.length == 1 ? '' : 's'}', style: const TextStyle(fontSize: 12, color: AppColors.textHint)),
            const Spacer(),
            GestureDetector(
              onTap: _createNewDeal,
              child: Container(
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  SizedBox(width: 4),
                  Text('New Deal', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.primary)),
                ]),
              ),
            ),
          ]),
      ...
      Expanded(
        child: _deals.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Text('No deals linked', style: TextStyle(color: AppColors.textHint)),
                const SizedBox(height: 12),
                FilledButton.icon(onPressed: _createNewDeal, icon: const Icon(Icons.add_rounded, size: 16), label: const Text('Create Deal')),
              ]))
            : ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  final contactName = d['contact']?['name'] ?? d['name'] ?? '';
                  final color = status == 'WON' ? AppColors.success : status == 'LOST' ? AppColors.error : AppColors.primary;
                  return GestureDetector(
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CrmDealDetailScreen(dealId: d['id'].toString(), dealName: contactName))),
                    child: Container(
                      margin: const EdgeInsets.only(bottom: 8),
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.surfaceLight)),
          ...
          itemBuilder: (_, i) {
            ...
            if (scheduled != null) {
              try {
                ...
                isOverdue = status == 'PENDING' && dt.isBefore(DateTime.now());
              } catch (_) {}
            }
            final isDone = status == 'COMPLETED' || status == 'DONE';
            final color = isDone ? AppColors.success : isOverdue ? AppColors.error : AppColors.warning;
            return Container(
              margin: const EdgeInsets.only(bottom: 8),
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: isOverdue && !isDone ? const Color(0xFFFEF2F2) : Colors.white,
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: isOverdue && !isDone ? const Color(0xFFFCA5A5) : AppColors.surfaceLight),
              ),
              child: Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    ...
                    Text(dateStr, style: TextStyle(fontSize: 10, color: isOverdue && !isDone ? AppColors.error : AppColors.textHint, fontWeight: isOverdue && !isDone ? FontWeight.w600 : FontWeight.normal)),
                  ]),
                  if (notes.toString().isNotEmpty) ...[
                    const SizedBox(height: 2),
