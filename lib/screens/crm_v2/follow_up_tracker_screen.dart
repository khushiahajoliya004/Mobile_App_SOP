...
class _FollowUpTrackerScreenState extends State<FollowUpTrackerScreen> {
  ...
  IconData _typeIcon(String type) {
    switch (type.toUpperCase()) {
      ...
      case 'VISIT': return Icons.home_rounded;
  ...
  String _contactOrDeal(Map<String, dynamic> f) {
    ...
    if (contact is Map && contact['name'] != null) return contact['name'].toString();
    final deal = f['deal'];
        ...
        builder: (ctx, setSS) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Row(children: [
                  Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.success.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.check_circle_outline, color: AppColors.success, size: 20)),
                  const SizedBox(width: 12),
                  const Text('Mark as Done', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
                ]),
                ...
                TextField(controller: notesCtrl, maxLines: 2, decoration: InputDecoration(labelText: 'Notes (optional)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)), contentPadding: const EdgeInsets.all(12))),
                ...
                const SizedBox(height: 20),
                FilledButton(
                  style: FilledButton.styleFrom(backgroundColor: AppColors.success, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                  onPressed: () async {
                    ...
                    try {
                      await _api.completeCrmFollowUp(f['id'].toString(), outcome: selectedOutcome!, notes: notesCtrl.text.trim().isEmpty ? null : notesCtrl.text.trim(), nextFollowUpDate: scheduleNext && nextDate != null ? nextDate!.toIso8601String() : null, nextFollowUpType: scheduleNext ? nextType : null);
                      if (selectedFile != null) {
                        try {
                          await _api.createCall(customerName: _contactOrDeal(f), companyId: _user?.companyId ?? '', userId: _user?.id ?? '', audioFilePath: selectedFile!.path, audioFileName: selectedFile!.name, followUpId: f['id'].toString());
                        } catch (_) {}
                      }
  ...
  Future<void> _showRescheduleSheet(Map<String, dynamic> f) async {
    DateTime selected = DateTime.tryParse(f['scheduledAt']?.toString() ?? '')?.toLocal() ?? DateTime.now().add(const Duration(hours: 1));
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSS) => Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              const Text('Reschedule', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
              const SizedBox(height: 16),
              GestureDetector(
                onTap: () async {
                  final d = await showDatePicker(context: ctx, initialDate: selected, firstDate: DateTime.now(), lastDate: DateTime.now().add(const Duration(days: 365)));
                  if (d != null && ctx.mounted) {
                    final t = await showTimePicker(context: ctx, initialTime: TimeOfDay.fromDateTime(selected));
                    if (t != null) setSS(() => selected = DateTime(d.year, d.month, d.day, t.hour, t.minute));
                  }
                },
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    Text('${selected.day}/${selected.month}/${selected.year}  ${_formatTime(selected.toIso8601String())}', style: const TextStyle(fontSize: 14, color: AppColors.textPrimary)),
                  ]),
                ),
              ),
              const SizedBox(height: 20),
              FilledButton(
                ...
                onPressed: () async {
                  try {
                    await _api.rescheduleCrmFollowUp(f['id'].toString(), selected.toUtc().toIso8601String());
                    if (ctx.mounted) Navigator.pop(ctx);
                    _load();
                  } catch (e) {
                    if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text('Failed: $e')));
                          ...
                          child: ListView(
                            ...
                            children: [
                              if (_pending.isNotEmpty) ...[
                                _sectionHeader('PENDING', _pending.length, AppColors.warning),
                                const SizedBox(height: 8),
                                ..._pending.map(_buildPendingCard),
                                const SizedBox(height: 16),
                              ],
                              if (_completed.isNotEmpty) ...[
                                _sectionHeader('COMPLETED', _completed.length, AppColors.success),
                                const SizedBox(height: 8),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                const Text('Follow-up Tracker', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
                Text(_formatDateHeader(_selectedDate), style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
              ],
            ),
          ),
          if (!_loading)
            Container(
              ...
              child: Text('$total total', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.primary)),
            ),
        ],
      ),
    );
          ...
          children: pills.map((p) {
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                child: Text(p.$2, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: sel ? Colors.white : AppColors.textSecondary)),
              ),
            );
          }).toList(),
        ),
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: [
                      ...
                      ..._branches.map((b) => DropdownMenuItem<String>(value: b['id']?.toString(), child: Text(b['name']?.toString() ?? '', style: const TextStyle(fontSize: 12)))),
                    ],
                    onChanged: (v) {
                      setState(() { _selectedBranchId = v; _selectedUserId = null; _users = []; });
              ...
              child: Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    hint: const Text('All People', style: TextStyle(fontSize: 12)),
                    isExpanded: true,
                    items: [
                      const DropdownMenuItem<String>(value: null, child: Text('All People', style: TextStyle(fontSize: 12))),
                      ..._users.map((u) => DropdownMenuItem<String>(
                        value: u['id']?.toString(),
                        child: Text('${u['firstName'] ?? ''} ${u['lastName'] ?? ''}'.trim(), style: const TextStyle(fontSize: 12)),
                      )),
                    ],
  ...
  Widget _sectionHeader(String title, int count, Color color) {
    return Container(
      ...
      child: Row(
        children: [
          Text('$title ($count)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: color)),
        ],
      ),
    );
  }
  ...
  Widget _buildPendingCard(Map<String, dynamic> f) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          left: BorderSide(color: overdue ? AppColors.error : AppColors.primary, width: 3),
        ),
          ...
          children: [
            Row(
              children: [
                Container(
                  width: 36, height: 36,
                  decoration: BoxDecoration(
                    color: (overdue ? AppColors.error : AppColors.primary).withValues(alpha: 0.12),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Icon(_typeIcon(type), size: 18, color: overdue ? AppColors.error : AppColors.primary),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(name, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary), overflow: TextOverflow.ellipsis),
                      if (subject.isNotEmpty) Text(subject, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary), overflow: TextOverflow.ellipsis),
                    ],
                  ),
                ),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(color: _priorityColor(priority).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(8)),
                  child: Text(priority, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: _priorityColor(priority))),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Row(
              children: [
                ...
                Text(time, style: TextStyle(fontSize: 11, color: overdue ? AppColors.error : AppColors.textSecondary, fontWeight: overdue ? FontWeight.w700 : FontWeight.normal)),
                if (overdue) ...[
                  const SizedBox(width: 6),
                  Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: AppColors.error.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(4)), child: const Text('OVERDUE', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: AppColors.error))),
                ],
                const Spacer(),
                if (assigned.isNotEmpty) ...[
                  const Icon(Icons.person_outline_rounded, size: 12, color: AppColors.textHint),
                  const SizedBox(width: 3),
                  Text(assigned, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                ],
              ],
            ),
            ...
            Row(
              children: [
                _actionBtn('Done', AppColors.success, Icons.check_rounded, () => _showCompleteSheet(f)),
                const SizedBox(width: 8),
                _actionBtn('Reschedule', AppColors.primary, Icons.schedule_rounded, () => _showRescheduleSheet(f)),
                const SizedBox(width: 8),
                _actionBtn('Cancel', AppColors.error, Icons.close_rounded, () => _cancelFollowUp(f)),
              ],
            ),
          ],
        ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(children: [
                  Expanded(child: Text(name, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700), overflow: TextOverflow.ellipsis)),
                  if (completedAt.isNotEmpty) Text(completedAt, style: const TextStyle(fontSize: 10, color: AppColors.textHint)),
                ]),
                if (outcome.isNotEmpty) ...[
                  const SizedBox(height: 2),
                  Text(outcome, style: const TextStyle(fontSize: 11, color: AppColors.success, fontWeight: FontWeight.w600)),
                ],
                if (completedBy.isNotEmpty) ...[
                  const SizedBox(height: 2),
                  Text('By $completedBy', style: const TextStyle(fontSize: 10, color: AppColors.textHint)),
                ],
              ],
            ),
          ),
