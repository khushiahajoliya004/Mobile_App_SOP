...
class _CrmDealsScreenState extends State<CrmDealsScreen> {
  ...
  Future<void> _init() async {
    ...
    debugPrint(
      '[CrmDeals] user=${user?.email}, branchId=${user?.branchId}, branchName=${user?.branchName}',
    );
  ...
  Future<void> _load({bool loadMore = false}) async {
    ...
    try {
      ...
      final res = await _api.getCrmDeals(
        ...
        branchId:
            _filterBranchId, // only explicit UI filter, never auto-injected
  ...
  List<Map<String, dynamic>> _normalizeBranchTeam(dynamic raw) {
    ...
    return (list as List)
        ...
        .where(
          (u) =>
              u['id'].toString().isNotEmpty && u['name'].toString().isNotEmpty,
        )
        .toList();
  }
  ...
  List<Map<String, dynamic>> get _currentPipelineStages {
    ...
    try {
      ...
      if (stages is List)
        return stages.map((s) => Map<String, dynamic>.from(s)).toList();
    } catch (_) {}
    return [];
  }
          ...
          Future<void> reloadBranchUsers(String? branchId) async {
            try {
              if (branchId != null) {
                ...
                setSheet(() {
                  branchUsers = _normalizeBranchTeam(res.data);
                  selectedOwner = null;
                  ownerSearch = '';
                ...
                final res = await _api.getAssignableUsers();
                setSheet(() {
                  branchUsers = _normalizeUsers(res.data);
                  selectedOwner = null;
                  ownerSearch = '';
                  showOwnerDropdown = false;
                });
                          ...
                          child: FilledButton(
                            onPressed: () async {
                              ...
                              if (phoneCtrl.text.trim().isEmpty) {
                                _msg('Phone is required', error: true);
                                return;
                              }
                              final phone = phoneCtrl.text.trim();
                              // Duplicate phone pre-check
                              try {
                                final dupRes = await _api
                                    .checkDuplicateCrmContact(phone);
                                ...
                                if (list is List &&
                                  ...
                                  final existingName = first is Map
                                      ? (first['name'] ?? 'another contact')
                                      : 'another contact';
                                  await showDialog<void>(
                                    ...
                                    builder: (dlgCtx) => AlertDialog(
                                      ...
                                      content: Text(
                                        'Phone $phone already exists as "$existingName". Please use a different number.',
                                      ),
                                      actions: [
                                        FilledButton(
                                          onPressed: () =>
                              ...
                              try {
                                final res = await _api.createCrmQuickLead(
                                  ...
                                  pipelineId: createDeal
                                      ? selectedPipelineId
                                      : null,
                                  expectedValue: createDeal
          ...
          children: [
            ...
            ..._branches.map(
              (b) => ListTile(
                contentPadding: EdgeInsets.zero,
                title: Text('${b['name']}'),
                trailing: _filterBranchId == b['id']
                    ? const Icon(Icons.check, color: AppColors.primary)
                    : null,
                onTap: () {
                  setState(() => _filterBranchId = b['id'] as String?);
                  Navigator.pop(ctx);
                  _load();
                },
              ),
                ...
                child: ListView(
                  ...
                  children: [
                    ListTile(
                      ...
                      trailing: _filterOwnerUserId == null
                          ? const Icon(Icons.check, color: AppColors.primary)
                          : null,
                      onTap: () {
                        setState(() => _filterOwnerUserId = null);
                        Navigator.pop(ctx);
                        _load();
                      },
                    ),
                    ..._assignableUsers.map(
                      (u) => ListTile(
                        ...
                        subtitle:
                            ...
                            : null,
                        trailing: _filterOwnerUserId == u['id']
                            ? const Icon(Icons.check, color: AppColors.primary)
                            : null,
                        onTap: () {
                          setState(
                            () => _filterOwnerUserId = u['id'] as String?,
                          );
                          Navigator.pop(ctx);
                          _load();
                        },
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () {
                        Navigator.pop(ctx);
                        _navigateToUpload(dealId, dealName, phone);
                      },
                  ...
                  Expanded(
                    child: FilledButton.icon(
                      onPressed: () {
                        Navigator.pop(ctx);
                        _navigateToRecorder(dealId, dealName, phone);
                      },
    ...
    for (final deal in allDeals) {
      final name =
          (deal['contact']?['name'] ??
                  ...
                  '')
              .toString();
      final phone = (deal['contact']?['phone'] ?? deal['phone'] ?? '')
          .toString();
      final email = (deal['contact']?['email'] ?? deal['email'] ?? '')
          .toString();
      final status = (deal['status'] ?? '').toString();
      final stage = (deal['stage']?['name'] ?? deal['stageName'] ?? '')
          .toString();
      final pipeline = (deal['pipeline']?['name'] ?? deal['pipelineName'] ?? '')
          .toString();
      final sourceRaw = (deal['contact']?['source'] ?? deal['source'] ?? '')
          .toString();
      final source =
          _leadSources.firstWhere(
            (s) => s['value'] == sourceRaw,
      ...
      final owner =
          '${deal['owner']?['firstName'] ?? ''} ${deal['owner']?['lastName'] ?? ''}'
              .trim();
      final branchId = (deal['branchId'] ?? deal['branch']?['id'] ?? '')
          .toString();
      final branch =
          (deal['branch']?['name'] ??
                  deal['branchName'] ??
      ...
      children: [
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: _pipelines.map((p) {
                ...
                return GestureDetector(
                  onTap: () {
                    setState(() => _selectedPipelineId = p['id'] as String?);
                    _load();
                  },
                  child: Container(
            ...
            children: [
              ...
              _buildFilterChip(
                label: _filterOwnerUserId != null
                    ? (_assignableUsers.firstWhere(
                                (u) => u['id'] == _filterOwnerUserId,
                                orElse: () => {'name': 'Owner'},
                              )['name']
                              as String? ??
                          'Owner')
        ...
        Padding(
          ...
          child: Row(
            children: [
              Text(
                _total > _deals.length
                    ? '${_deals.length} of $_total deals'
                    : '${_deals.length} deals',
  ...
  Widget _dealTile(Map<String, dynamic> deal) {
    ...
    final aiScore = deal['aiScore'] != null
        ? (deal['aiScore'] as num).toInt()
        : 0;
    final stageEnteredAt = deal['stageEnteredAt'] != null
        ? DateTime.tryParse(deal['stageEnteredAt'].toString())
        : null;
    final daysInStage = stageEnteredAt != null
        ? DateTime.now().difference(stageEnteredAt).inDays
        : null;
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        _tinyAction(
                          ...
                          color: AppColors.success,
                          onTap: () => _showCallSummaryForDeal(
                            deal['id'].toString(),
                            name,
                            hasNoCalls: callCount == 0,
                          ),
                        ),
    ...
    return Material(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: Scaffold(backgroundColor: AppColors.scaffoldBg, body: child),
          ),
        ],
      ),
    );
...
class _DealCallSummarySheetState extends State<_DealCallSummarySheet> {
      ...
      builder: (_, scrollCtrl) {
        return Container(
          ...
          child: _loading
              ...
              ? Center(
                  child: Text(
                    _error ?? 'No calls found for this deal',
                        ...
                        Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 8),
                              Text(
                                _selectedDetail!['analysisStatus'] ==
                                            'MANUAL' ||
                                        _selectedDetail!['analysisStatus'] ==
                                            'SKIPPED'
  ...
  bool _noContent(Map<String, dynamic> d) {
    ...
    final rawSummary =
        d['callSummary'] ?? ai['summary'] ?? ai['callSummary'] ?? '';
  ...
  Widget _buildHeader(Map<String, dynamic> d) {
    final score = d['sopScore'] != null
        ? num.tryParse(d['sopScore'].toString())
        : null;
