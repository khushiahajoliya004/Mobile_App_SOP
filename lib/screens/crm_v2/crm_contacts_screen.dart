...
class _CrmContactsScreenState extends State<CrmContactsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (_filterStage != null) {
        _contacts = _contacts
            .where((c) => c['lifecycleStage'] == _filterStage)
            .toList();
      }
    } catch (_) {
      _contacts = [];
  ...
  void _showCreateLeadForm() {
    ...
    final branchLocked =
        _currentUserBranchId != null && _currentUserBranchId!.isNotEmpty;
          ...
          Future<void> reloadBranchUsers(String? branchId) async {
            try {
              if (branchId != null) {
                ...
                setSheet(() {
                  branchUsers = List.from(_assignableUsers);
                  selectedOwner = null;
                  ownerSearch = '';
                  showOwnerDropdown = false;
                });
                          ...
                          child: FilledButton(
                            onPressed: () async {
                              ...
                              try {
                                ...
                                _msg(extractErrorMessage(e), error: true);
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await _api.deleteCrmContact(contact['id']);
                _msg('Contact deleted');
                _load();
              } catch (e) {
                _msg(extractErrorMessage(e), error: true);
              }
