...
class _IndustriesScreenState extends State<IndustriesScreen> {
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _field(nameCtrl, 'Industry Name', Icons.factory_rounded),
              ...
              child: DecoratedBox(
                ...
                child: FilledButton(
                  onPressed: () async {
                    ...
                    try {
                      if (isEdit) {
                        await _api.updateIndustry(
                          existing['id'],
                          name: nameCtrl.text.trim(),
                          description: descCtrl.text.trim(),
                        );
                        _msg('Industry updated');
                      } else {
                        await _api.createIndustry(
                          name: nameCtrl.text.trim(),
                          description: descCtrl.text.trim(),
                          companyId: _companyId.isNotEmpty ? _companyId : null,
                        );
                        _msg('Industry created');
                      }
                      _load();
                    } catch (e) {
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await _api.deleteIndustry(item['id']);
                _msg('Deleted');
                _load();
              } catch (e) {
                _msg('Failed: $e', error: true);
  ...
  Future<void> _toggleStatus(Map<String, dynamic> item) async {
    ...
    try {
      await _api.updateIndustryStatus(item['id'], newStatus);
      _msg('Status updated');
      _load();
    } catch (e) {
      _msg('Failed: $e', error: true);
